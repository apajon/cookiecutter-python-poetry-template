...
bake-src: ## bake without inputs and overwrite if exists.
	@poetry run cookiecutter --no-input . --overwrite-if-exists layout="src"

.PHONY: bake-with-inputs
bake-with-inputs: ## bake with inputs and overwrite if exists.
	@poetry run cookiecutter . --overwrite-if-exists
...
test: ## Test the code with pytest.
	...
	@poetry run python -m pytest --cov --cov-config=pyproject.toml --cov-report=xml tests
...
clean-build: ## Clean build artifacts
	...
	@poetry run python -c "import shutil; import os; shutil.rmtree('dist') if os.path.exists('dist') else None"
